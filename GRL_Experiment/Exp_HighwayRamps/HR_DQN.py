...
class Experiment:
    ...
    def run(self, num_HVs, num_AVs, training, testing, Graph):
        ...
        from GRL_Library.agent.Discrete import DQN_agent
        ...
        if Graph:
            from GRL_Net.Model_Discrete.Q_Net import Graph_Model
            GRL_Net = Graph_Model(N, F, A)
        else:
            from GRL_Net.Model_Discrete.Q_Net import NonGraph_Model
            GRL_Net = NonGraph_Model(N, F, A)

        # Initialize optimizer
        optimizer = torch.optim.Adam(GRL_Net.parameters(), eps=0.0001)
        # Replay_buffer
